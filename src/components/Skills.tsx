...
export default function Skills() {
  ...
  const [headerRef, headerVisible] = useScrollFade<HTMLDivElement>({ threshold: 0.2 });
  const [gridRef, gridVisible]     = useScrollFade<HTMLDivElement>({ threshold: 0.04 });
        ...
        >
          {skills.map((group, i) => (
            <motion.div
              key={group.category}
              initial={{ opacity: 0, y: 40 }}
              animate={{ opacity: gridVisible ? 1 : 0, y: gridVisible ? 0 : 40 }}
